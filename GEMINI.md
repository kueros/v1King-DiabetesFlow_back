...
- **Sesión 2026-05-22 (VIII):** Implementación del motor clínico simplificado en `CalculatorService` para cálculo de bolos y despliegue del endpoint en `CalculatorController` extrayendo parámetros explícitos (`@Body`). Refactorización estricta de CRUD para `ActivitiesService` y `ActivitiesController` con tipado de Prisma sin dependencias opcionales. Creación del módulo `GlucoseModule` con servicio y controlador para registro de glucemias; se añadió validación estricta (`isNaN`) y mapeo de parámetros `glucose`/`context` hacia `level`/`context` del esquema. Configuración de soporte JSX en backend (`tsconfig.json`) y scaffolding de página frontend de Next.js (`src/app/glucose/page.tsx`, `src/components/GlucoseLogger.tsx`) con estética v1King y Server Actions simuladas. Archivos afectados: `calculator.service.ts`, `calculator.controller.ts`, `calculator.module.ts`, `activities.service.ts`, `activities.controller.ts`, `glucose.module.ts`, `glucose.service.ts`, `glucose.controller.ts`, `tsconfig.json`, `page.tsx`, `GlucoseLogger.tsx`. Pendiente: Algoritmo IOB e Integración Frontend Completa.
- **Sesión 2026-05-22 (IX):** Corrección y refactorización de decoradores REST en `GlucoseController`. Verificación del decorador global `@Controller('glucose')`. Sustitución del decorador parametrizado `@Get('user/:userId')` por el decorador raíz `@Get()` y extracción de variable mediante `@Query('userId')` importado nativamente para mantener consistencia de ruta sin afectar la lógica de negocio. Archivos afectados: `glucose.controller.ts`. Pendiente: Algoritmo IOB e Integración Frontend Completa.
